...
SITUATIONS = [
    ...
    ("You need a statement for a customer",
     "Quotes > the Statement button (on the Quotes list toolbar, on a quote's detail screen, and it opens by itself when you mark a quote Accepted). The statement lists the latest document per quote - Tax Invoice first, then Pro-Forma, then an accepted Quote. Lines are editable, and an adjustment line with a MINUS amount is how you enter a payment or a credit. Then print the PDF."),
    ("You want to invoice a deposit now and the balance later",
     "On the quote's detail screen tick 'Split deposit / balance'. The single Tax Invoice button is replaced by two: Deposit Invoice and Balance Invoice. Generate the deposit one FIRST - the balance invoice deducts it and will refuse until the deposit exists. Each is a real Tax Invoice with its own number."),
    ("The deposit is not the percentage you want",
     "Settings > Business Details > 'Deposit % (balance is the rest)'. Set it to any whole number from 1 to 99; the balance is always what is left over. It changes the split on every NEW deposit/balance invoice and the wording on the buttons. Invoices already raised keep the split they were raised at."),
    ("You want to cost a job but there is no job to pick",
     "Job Costing > '+ New Job'. Jobs used to be created from the quote screen; that was removed when the app was made generic, so Job Costing now opens its own jobs. Pick the customer, give it a PO or reference, and it appears in the list ready for revenue and cost lines. The cost items themselves live in Settings > Job Costing."),
    ...
    ("A bank statement imported without categories",
     "That is now the normal, expected result. The app ships with almost no built-in rules - it categorises from rules YOU save. Change one transaction's category and say yes when asked to remember the rule; the next import applies it automatically. See the Auto-Categorising sheet."),
    ("The app opens small / not full screen",
     "Click the 'Full screen' button in the top bar. It is on every page now, not just the Dashboard, so you no longer have to go back to the Dashboard to get it. This is a known issue with the startup state on some PCs - the button is the fix."),
...
AUTO_CATEGORY = [
    ...
    ("Why there are almost no built-in rules",
     "There used to be a long built-in list of payees and suppliers. It belonged to one specific set of bank accounts, so on anyone else's statement it put rows in the wrong place. It was removed. Only bank-generated descriptions are still matched automatically - account and service fees, and SARS. Everything else waits for your own saved rules. Your bank also categorises its own statements, which is a second place to look."),
    ("Any format of bank statement still imports",
     "Nothing changed about importing. Any statement the app could read before, it still reads. Only the automatic categorising afterwards was cut back."),
    ("How to teach it",
    ...
    ("How shops are matched",
     "The app strips card numbers and dates off the description, so the same shop on the 24th and on the 28th - each with its own card reference and date tacked on the end - counts as one shop. Slight variations in the reference do not break it."),
...
def main():
    ...
    _table(
        ...
        [("2026-09-23", "Claude", "All 11 checks PASS",
          "Ops Hub is now fully generic. Shade-netting pricing tools, the netting quote builder and the Job KPI window were removed, and so were all the Job Card screens - Job Costing keeps its own '+ New Job' button instead. Statements editor restored (Quotes > Statement). Split deposit/balance Tax Invoices added, with the deposit percentage now editable in Settings > Business Details (default 65, balance is the rest) - database migration v0058. Full-screen button moved to the top bar so it is on every page. Bank-import auto-categorising rules for named payees and suppliers removed; statements import uncategorised until you save your own rules. Modules: Dashboard, CRM, Quotes, Accounting, Accounting Workbook, Communications, Documents, Gallery, Calendar/Jobs, Projects, Job Costing, Proposals, Annual Compliance, Backup, Settings, Troubleshooter, Batch Renamer, Duplicate Finder, Empty Folder Remover, File Mover. No Site Visit module."),
