"""Export the FC Hub troubleshooting guide to Excel.

Plain-language: what each diagnostic check means, what to do when it
fails, and whether it is safe to keep working. Written for Minette to
use on her own without waiting for help.
...
CHECKS = [
    ("Project structure",
     "One of the app's core folders or files is missing (assets, core, modules, database).",
     rf"Almost always means files were moved or only part of the folder was copied. Restore the newest folder from {ROOT}\backups, or re-copy the whole FC_Hub folder. Do not just recreate an empty folder - the contents are what matter.",
    ...
    ("Module discovery report",
     ...
     rf"Same causes and fix as Module registration. Check {ROOT}\modules still has its subfolders and each contains a module.py.",
    ...
    ("Application folder access",
     ...
     "Usually Windows permissions, or OneDrive locking the folder, or a file open in Excel. Close any open export and retry. If it persists, check FC_Hub is not inside a synced OneDrive folder.",
     "No - saving will fail."),
    ("Database health",
     "The database is corrupt, unreachable, or has integrity / foreign-key errors.",
     rf"STOP. Do not keep entering data. Restore the newest {ROOT}\database\fc_hub.db.bak_* file over fc_hub.db, or restore a full backup folder. Tell Claude before doing anything else.",
     "No - stop immediately."),
    ("Backup services",
     "The backup system cannot reach its folder.",
     rf"Make sure {ROOT}\backups exists and is writable. Same OneDrive / permissions causes as folder access.",
     "Yes, but you are working unprotected - fix it soon."),
    ("Paths and resources",
     "The logo, fonts or other bundled assets cannot be found.",
     rf"In a rebuilt .exe this usually means the assets folder was not bundled - documents would then print with no logo and in the wrong font. Ask Claude to rebuild. From source, check {ROOT}\assets still exists.",
...
SITUATIONS = [
    ("The app will not open at all",
     rf"1. Try opening it once more - a stuck previous copy can block it." "\n"
     r"2. Restart Windows." "\n"
     rf"3. Still failing: restore the newest folder from {ROOT}\backups." "\n"
     r"4. Tell Claude exactly what the error said."),
    ...
    ("A price is wrong on a quote",
     "Settings > Supplier Pricing. Edit the item's cost, then press Calculate Prices on the quote again. You do not need Claude for this."),
    ("You changed a supplier price and nothing happened",
     "Prices apply to NEW calculations. Reopen the quote and press Calculate Prices again. Quotes already issued deliberately keep their original figures - that is correct, not a bug."),
    ...
    ("Before ANY big change",
     "Make a backup: the Backup module, or copy the whole FC_Hub folder. Backups are free; recovery is not."),
]
...
def main():
    destination = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("FC_Hub_Troubleshooting_Guide.xlsx")
    ...
    _table(
        ...
        [("2026-08-09", "Claude", "All 11 checks PASS",
          "Baseline. Database v0035, integrity ok, foreign keys enabled, all 17 modules registered.")],
        [14, 16, 24, 84], 34,
    )
